#!/usr/bin/perl
#
# Grasshopper Polling driver for a generic switch with 64 bit counters
# 
...
sub poll
{
    ...
    my $ifXtabledata = $session->gettable('ifXTable');
	...
	my $ifXvalue = $ifXtabledata->{$key};
	my $mergedValue = { %$value, %$ifXvalue };	
	my $xml = XML::Dumper::pl2xml($mergedValue);
        DriverHelpers::xml_dump($mergedValue,$xmlpath,$xml);
    }
...
sub update_rrd
{
    ...
    unless(-e $rrdfile) {
        ...
        $rrd->create(
                ...
                zmulticastInPerSec => "Counter",
                zmulticastOutPerSec => "Counter",
                zbroadcastInPerSec => "Counter",
                zbroadcastOutPerSec => "Counter",
                errorsInPerSec => "Counter",
                errorsOutPerSec => "Counter"
                );
    }

    my $octetsIn    = $ifdata->{'ifHCInOctets'};
    my $octetsOut   = $ifdata->{'ifHCOutOctets'};
    my $ucastIn     = $ifdata->{'ifHCInUcastPkts'};
    my $ucastOut    = $ifdata->{'ifHCOutUcastPkts'};
    my $zmulticastIn  = $ifdata->{'ifHCInMulticastPkts'};
    my $zmulticastOut = $ifdata->{'ifHCOutMulticastPkts'};
    my $zbroadcastIn  = $ifdata->{'ifHCInBroadcastPkts'};
    my $zbroadcastOut = $ifdata->{'ifHCOutBroadcastPkts'};
    ...
    $rrd->update("$rrdfile",time(),
            ...
            zmulticastInPerSec  => $zmulticastIn,
            zmulticastOutPerSec => $zmulticastOut,
            zbroadcastInPerSec  => $zbroadcastIn,
            zbroadcastOutPerSec => $zbroadcastOut,
